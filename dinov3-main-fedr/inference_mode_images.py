...
parser.add_argument("--output_path", type=str, default="/uoa/scratch/users/r02sw23/dinov3-main-fedr/results/8_RF/")
parser.add_argument("--filename", type=str, default="/uoa/scratch/users/r02sw23/dinov3-main-fedr/RF_models/2/fg_classifier100_est.pkl")
# ------------------------------------------------------------------------
...
def main():
    ...
    wandb.init(project=f"DINOv3 Segmentation Inference Images {args.model_type} Test",
                name=f"Borebreen Images 1 to 10")
    ...
    for i in range(args.start_img, args.end_img+1):
        
        test_image_dir = args.image_dir + f'borebreen_crop_drone_{i}.png'
        test_label_dir = args.label_dir + f'borebreen_crop_drone_{i}.png'
    ...
    overall_dsc = dsc_score/ float(args.end_img)
    overall_iou = iou_score/ float(args.end_img)
    overall_acc = acc_score/ float(args.end_img)
    overall_pre = pre_score/ float(args.end_img)
    overall_rec = rec_score/ float(args.end_img)

    # Check the accuracy on the test dataset. If this is too low compared to train it indicates overfitting on the training data
    print("Average Dice Score Coefficiant = ", overall_dsc)
    print("mIoU = ", overall_dsc)
    print("Overall Accuracy = ", overall_dsc)
    print("Overall Precision = ", overall_dsc)
    print("Overall Recall = ", overall_dsc)
