...
parser.add_argument("--images_no", type=int, default=1)
...
def main():
    ...
    wandb.init(project=f"DINOv3 Segmentation Head m2f {args.model_type} Images 896 Test",
    ...
    # Save results
    plt.savefig(args.output_dir + 'subplot_results.png')
    plt.close()
    print(f"Results saved to: {args.output_dir}")
    
    # Also save individual images
    img.save(args.output_dir + 'original_image.png')
    
    # Save segmentation map as image
    seg_img = Image.fromarray(
        (segmentation_map_vit7b[0, 0].cpu().numpy() * 255 / 150).astype('uint8')
    )
    seg_img.save(args.output_dir + 'segmentation_map.png')
