...
# Input parameters
# parser.add_argument("--model_type", type=str, default='ViT7b-natural')
parser.add_argument("--model_type", type=str, default='LR')
parser.add_argument("--max_block_no", type=int, default=39)
parser.add_argument("--total_imgs", type=int, default=40)
parser.add_argument("--image_size", type=int, default=512)
parser.add_argument("--image_dir", type=str,
                    default="/uoa/scratch/users/r02sw23/borebreen-drone-image-data-test/images/")
parser.add_argument("--label_dir", type=str,
                    default="/uoa/scratch/users/r02sw23/borebreen-drone-image-data-test/masks/")
parser.add_argument("--output_path", type=str,
                    default="/uoa/scratch/users/r02sw23/dinov3-foreground-segmentation-tests-natural/test_results/")

parser.add_argument("--filename", type=str,
                    default="/uoa/scratch/users/r02sw23/dinov3-foreground-segmentation-tests-natural/fg_classifier_natural.pkl")

parser.add_argument("--model_path", type=str,
                    default='/uoa/scratch/users/r02sw23/dinov3-main/pre_trained_weights/dinov3_vit7b16_pretrain_lvd1689m-a955f4ea.pth')
...
def main():
    ...
    wandb.init(project=f"DINOv3 Natural {args.model_type} ForeGround Segmentation Tests Maxwell",
               name=f"Borebreen Images 1 to {int(args.total_imgs/4)}, {args.image_size}")
    ...
    load_data = LoadData(args.image_dir, args.label_dir, args.image_size)
    test_images, test_labels = load_data.sequence_data_loading()

    print('Number of Test Images: ', len(test_images))
    print('Number of Test Labels: ', len(test_labels))

    dsc_score, iou_score, acc_score, pre_score, rec_score = 0., 0., 0., 0., 0.

    for i in range(args.total_imgs):
        ...
        plt.savefig(args.output_path + f'model_outputs_plot_borebreen{i}.png')
        plt.close()
        ...
        # Add this right after line 163 (after y_pred calculation)
        print(f"Ground truth positive pixels: {mask_i_flat.sum()}")
        print(f"Predicted positive pixels: {y_pred_flat.sum()}")
        print(f"True Positives: {((mask_i_flat == 1) & (y_pred_flat == 1)).sum()}")
        print(f"False Positives: {((mask_i_flat == 0) & (y_pred_flat == 1)).sum()}")
        print(f"False Negatives: {((mask_i_flat == 1) & (y_pred_flat == 0)).sum()}")
        ...
        plt.imshow(y_pred)
        plt.title('foreground score')
        plt.savefig(args.output_path + f'model_predictions_plot_borebreen{i}.png')
        plt.close()

        plt.imshow(y_pred)
        plt.axis('off')
        plt.savefig(args.output_path + f'model_predictions_borebreen{i}.png')
        # plt.show()
        plt.close()

    overall_dsc = dsc_score / float(args.total_imgs)
    overall_iou = iou_score / float(args.total_imgs)
    overall_acc = acc_score / float(args.total_imgs)
    overall_pre = pre_score / float(args.total_imgs)
    overall_rec = rec_score / float(args.total_imgs)

    # Check the accuracy on the test dataset. If this is too low compared to train it indicates overfitting on the training data
    print("\nAverage Dice Score Coefficiant = ", overall_dsc)
    print("mIoU = ", overall_iou)
    print("Overall Accuracy = ", overall_acc)
    print("Overall Precision = ", overall_pre)
    print("Overall Recall = ", overall_rec)
