...
test_image_path = "/uoa/scratch/users/r02sw23/borebreen-drone-image-data-test/images/"    # Test images and inferrence
test_mask_path = "/uoa/scratch/users/r02sw23/borebreen-drone-image-data-test/masks/"      # Test masks and inferrence
...
##############################################################################################
def load_test_labels(test_mask_path, image_path):
    
    img = Image.open(test_mask_path + image_path).convert("L")
...
for i in range(start_img, end_img+1):
    ...
    image_path = f'borebreen_crop_drone_{i}.png'
    test_image = Image.open(test_image_path + image_path).convert("RGB")
    ...
    plt.savefig(output_path + f'model_outputs_plot_borebreen{i}.png')
    plt.close()
    
    y_pred = clf.predict(x)[:, 1].reshape(h_patches, w_patches)
    y_test = load_test_labels(test_mask_path, image_path)
...
overall_dsc = dsc_score/ float(end_img)
overall_iou = iou_score/ float(end_img)
overall_acc = acc_score/ float(end_img)
overall_pre = pre_score/ float(end_img)
overall_rec = rec_score/ float(end_img)

# Check the accuracy on the test dataset. If this is too low compared to train it indicates overfitting on the training data
print("Average Dice Score Coefficiant = ", dsc)
print("mIoU = ", iou)
